...
//classe que representa cada rota/caminho entre cada dois vértices/pontos
class Aresta
{
public:
    int v1;
...
//função utilizada para setar os valores no grafo,
...
//função que recebe dois pontos e busca qual a posição da aresta no vetor
...
//função principal,calcula probabilidades, caminhos, distancia e niveis de feromonio
void retornarRota(int numIteracoes, int numVertices, int numArestas, Aresta A[])
{
    ...
    while(numIteracoes > 0)
    {
        ...
        for(int v = 0; v < numVertices; v++)
        {
            ...
            //laço que limpa os vetores utilizados
            ...
            //laço responsavel por tracar uma rota
            while(cont <= numVertices)
            {
                cont ++;
                somatorio = 0;

                //laço que faz o somatorio usado no calculo da probabilidade
                for(i = 0; i < numVertices; i++)
                {
                ...
                //laço que calcula a probabilidade, e em seguida desloca para o próximo vértice
                for(i = 0; i < numVertices; i++)
                {
                    if(verticeAtual != i && verticesVisitados[i] == 0)
                    {
                        ...
                        //condicional para simular o grafico de pizza, fazendo uma escolha aleatória,
            ...
            //atualizar feromonio de cadas trecho, guardar em uma auxiliar, e só atualizar no original após rodar todos os vértices
            for(i = 0; i < 5; i++)
            {
...
void Grafo1(int numeroIte)
{
    ...
    //armazena as informações de cada caminho, não faz a duplicaçãode caminhos
